...
class PhiAttention(nn.Module):
    ...
    def __init__(self, config: PhiConfig, layer_idx: Optional[int] = None, add_gate: bool = False):
        super().__init__()
        self.config = config
        ...
        if self.add_gate:
            self.gate = nn.Linear(config.hidden_size, self.num_heads)
            # set bias to 10, so that the gate is close to 1 initially
            with torch.no_grad():
    ...
    def forward(
        ...
        if self.add_gate:
            # gate: [B, L, heads], x: [B, L, width] -> reshape x to [B, L, heads, head_dim]
            gate = torch.sigmoid(self.gate(attn_output))  # [B, L, heads]
            head_dim = self.head_dim // self.num_heads
            attn_output = attn_output.view(*attn_output.shape[:-1], self.num_heads, head_dim) * gate.unsqueeze(-1)
            attn_output = attn_output.view(*attn_output.shape[:-2], self.hidden_size)
...
class PhiSdpaAttention(PhiAttention):
    def __init__(self, *args, add_gate: bool = False, **kwargs):
        super().__init__(*args, add_gate=add_gate, **kwargs)
...
class PhiDecoderLayer(nn.Module):
    def __init__(self, config: PhiConfig, layer_idx: int, add_gate: bool = False):
        super().__init__()
        self.self_attn = PHI_ATTENTION_CLASSES[config._attn_implementation](config, layer_idx=layer_idx, add_gate=add_gate)
...
class PhiModel(PhiPreTrainedModel):
    ...
    def __init__(self, config: PhiConfig, add_gate: bool = False):
        ...
        self.layers = nn.ModuleList(
            [PhiDecoderLayer(config, layer_idx, add_gate=add_gate) for layer_idx in range(config.num_hidden_layers)]
        )
...
class PhiForCausalLM(PhiPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config, add_gate: bool = False):
        super().__init__(config)
        config.qk_layernorm = True
        config.use_cache = False
        self.model = PhiModel(config, add_gate=add_gate)
        self.vocab_size = config.vocab_size
    ...
    def forward(
        ...
        output_attentions: Optional[bool] = None,
        output_hidden_states: Optional[bool] = None,
        early_output: Optional[bool] = None,
        return_dict: Optional[bool] = None,
