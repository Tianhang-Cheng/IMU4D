"""
References: https://github.com/brentyi/egoallo/blob/main/src/egoallo/preprocessing/geometry/rotation.py
"""
...
def calculate_relative_rotation(absolute_rot: torch.Tensor, src_rep: str, tgt_rep: str=None) -> torch.Tensor:
    """
    Calculates the relative rotation vectors between consecutive time steps.
    
    The relative rotation R_rel(t) is the rotation that transforms from the
    orientation at time t-1 to the orientation at time t.
    R_abs(t) = R_rel(t) @ R_abs(t-1)
    => R_rel(t) = R_abs(t) @ R_abs(t-1)^T

    Args:
        absolute_rot: A tensor of absolute rotation vectors with shape [batch, n_time, n_var, 3].

    Returns:
        A tensor of relative rotation vectors with the same shape. For t=0,
        the original rotation vector is returned.
    """

    assert src_rep in ['aa', '6d'], f"Unknown rot_rep: {src_rep}"
    if tgt_rep is None:
        tgt_rep = src_rep
    assert tgt_rep in ['aa', '6d', 'mat'], f"Unknown tgt_rep: {tgt_rep}"

    rot_dof = 3 if src_rep == 'aa' else 6
    tgt_rot_dof = 3 if tgt_rep == 'aa' else 6

    batch, n_time, n_var, _ = absolute_rot.shape
    
    # Convert all absolute rotation vectors to rotation matrices
    # Reshape to [batch * n_time * n_var, 3] for batch processing
    flat_rot = absolute_rot.view(-1, rot_dof)
    flat_abs_mats = convert_rotation(flat_rot, src_rep, 'mat')
    abs_mats = flat_abs_mats.view(batch, n_time, n_var, 3, 3)

    # The rotation at t-1, shifted to align with t
    # For t=0, there's no t-1, so we use identity matrix, making R_rel(0) = R_abs(0)
    prev_abs_mats = torch.cat([
        torch.eye(3, device=abs_mats.device)
             .view(1, 1, 1, 3, 3)
             .expand(batch, 1, n_var, -1, -1).to(abs_mats.dtype),
        abs_mats[:, :-1]
    ], dim=1)
    
    # Calculate relative rotation matrices: R_rel = R_abs(t) @ R_abs(t-1)^T
    # Transpose is equivalent to inverse for rotation matrices
    relative_mats = torch.matmul(abs_mats, prev_abs_mats.transpose(-2, -1))
    
    # Convert the first time step's relative matrix back to the original absolute vector
    # This is because for t=0, R_rel(0) = R_abs(0)
    relative_mats[:, 0] = abs_mats[:, 0]
    
    # Convert relative matrices back to original rotation representation
    flat_relative_mats = relative_mats.view(-1, 3, 3)
    if tgt_rep == 'mat':
        relative_rots = flat_relative_mats.view(batch, n_time, n_var, 3, 3)
    else:
        flat_relative_rots = convert_rotation(flat_relative_mats, 'mat', tgt_rep)
        relative_rots = flat_relative_rots.view(batch, n_time, n_var, tgt_rot_dof)

    return relative_rots

def recover_absolute_rotation(relative_rot: torch.Tensor, rot_rep: str) -> torch.Tensor:
    """
    Recovers the absolute rotation vectors from a sequence of relative rotations.
    
    Inverse of `calculate_relative_rotation`.
    R_abs(t) = R_rel(t) @ R_abs(t-1)

    Args:
        relative_rot: A tensor of relative rotation vectors with shape 
                      [batch, n_time, n_var, rot_dof], where rot_dof is 3 ('aa') or 6 ('6d').
        rot_rep: Rotation representation, either 'aa' (axis-angle) or '6d'.

    Returns:
        A tensor of absolute rotation vectors with the same shape.
    """
    assert rot_rep in ['aa', '6d'], f"Unknown rot_rep: {rot_rep}"
    rot_dof = 3 if rot_rep == 'aa' else 6

    batch, n_time, n_var, _ = relative_rot.shape
    
    # Convert relative rotation to matrices
    flat_rel = relative_rot.reshape(-1, rot_dof)
    flat_rel_mats = convert_rotation(flat_rel, rot_rep, 'mat')
    rel_mats = flat_rel_mats.view(batch, n_time, n_var, 3, 3)
    
    # Initialize absolute rotation list
    absolute_mats_list = []
    current_abs_mat = rel_mats[:, 0]   # R_abs(0) = R_rel(0)
    absolute_mats_list.append(current_abs_mat)
    
    # Iteratively compute absolute rotations
    for t in range(1, n_time):
        # R_abs(t) = R_rel(t) @ R_abs(t-1)
        current_abs_mat = torch.matmul(rel_mats[:, t], current_abs_mat)
        absolute_mats_list.append(current_abs_mat)
    
    # Stack into [batch, n_time, n_var, 3, 3]
    absolute_mats = torch.stack(absolute_mats_list, dim=1)
    
    # Convert back to original representation
    flat_abs_mats = absolute_mats.view(-1, 3, 3)
    flat_abs_rots = convert_rotation(flat_abs_mats, 'mat', rot_rep)
    absolute_rots = flat_abs_rots.view(batch, n_time, n_var, rot_dof)
